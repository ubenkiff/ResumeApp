  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setEditingItem({}); setFormData({}); setImageUrls([]); }} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">+ Add Project</button>
      </div>
      ...
      {editingItem !== null && (
        <form onSubmit={handleSubmit} className="bg-slate-800 rounded-2xl shadow-xl p-6 mb-8 border border-slate-700">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Project Images (select multiple)</label>
              <input type="file" accept="image/*" multiple onChange={(e) => handleMultipleImageUpload(Array.from(e.target.files))} className="w-full" />
              {uploading && <p className="text-blue-400 text-sm mt-1">Uploading images...</p>}
              {imageUrls.length > 0 && (
            ...
            <label className="flex items-center gap-2">
              ...
              <span className="text-slate-300">Feature this project on portfolio</span>
            </label>
          </div>
          <div className="mt-6 flex gap-3">
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg transition">Save</button>
            <button type="button" onClick={() => { setEditingItem(null); setFormData({}); setImageUrls([]); }} className="bg-slate-700 hover:bg-slate-600 text-slate-300 px-6 py-2.5 rounded-lg transition">Cancel</button>
          </div>
        </form>
      )}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {projects.map((project) => (
          <div key={project.id} className="bg-slate-800 rounded-2xl shadow-xl p-4 border border-slate-700">
            {project.image_urls && project.image_urls.length > 0 && (
              <div className="flex gap-2 mb-3 overflow-x-auto">
                {project.image_urls.slice(0, 3).map((url, idx) => (
                  <img key={idx} src={url} className="w-16 h-16 object-cover rounded cursor-pointer" onClick={() => window.open(url, '_blank')} />
                ))}
              </div>
            )}
            ...
            <div className="flex justify-between items-center mt-3">
              <div className="flex gap-2">
                {project.live_url && <a href={project.live_url} target="_blank" className="text-blue-400 text-sm">Live</a>}
              </div>
              <div className="flex gap-2">
                <button onClick={() => handleEdit(project)} className="text-blue-400 text-sm">Edit</button>
                <button onClick={() => handleDelete(project.id)} className="text-red-400 text-sm">Delete</button>
              </div>
            </div>
          </div>
        ))}
