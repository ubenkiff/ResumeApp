  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setEditingItem({}); setFormData({}); }} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">+ Add Achievement</button>
      </div>
      ...
      {editingItem !== null && (
        <form onSubmit={handleSubmit} className="bg-slate-800 rounded-2xl shadow-xl p-6 mb-8 border border-slate-700">
          ...
          <div className="space-y-3">
            ...
            <select value={formData.category || ''} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full p-3 bg-slate-900 border border-slate-700 rounded-lg text-white">
      ...
      <div className="space-y-3">
        {achievements.map((ach) => (
          <div key={ach.id} className="bg-slate-800 rounded-2xl shadow-xl p-5 border border-slate-700">
            <div className="flex justify-between items-start">
              <div>
                ...
                {ach.description && <p className="text-slate-300 text-sm mt-1">{ach.description}</p>}
                {ach.category && <span className="inline-block mt-2 px-2 py-1 bg-slate-700 rounded text-xs text-slate-300">{ach.category}</span>}
              </div>
              <div className="flex gap-2">
                <button onClick={() => { setEditingItem(ach); setFormData(ach); }} className="text-blue-400">Edit</button>
                <button onClick={() => handleDelete(ach.id)} className="text-red-400">Delete</button>
              </div>
            </div>
          </div>
        ))}
        {achievements.length === 0 && (
          <p className="text-slate-400 text-center py-8">No achievements yet. Click "Add Achievement" to get started.</p>
        )}
      </div>
    </div>
  );
