  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setEditingItem({}); setFormData({}); }} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">+ Add Skill</button>
      </div>
      ...
      {editingItem !== null && (
        <form onSubmit={handleSubmit} className="bg-slate-800 rounded-2xl shadow-xl p-6 mb-8 border border-slate-700">
          ...
          <div className="space-y-3">
            ...
            <select value={formData.level || ''} onChange={(e) => setFormData({ ...formData, level: e.target.value })} className="w-full p-3 bg-slate-900 border border-slate-700 rounded-lg text-white">
      ...
      {categories.map(category => (
        <div key={category} className="mb-6">
          <h2 className="text-lg font-semibold text-blue-400 mb-3">{category}</h2>
          <div className="space-y-2">
            {skills.filter(s => s.category === category).map((skill) => (
              <div key={skill.id} className="bg-slate-800 rounded-lg shadow p-4 flex justify-between items-center border border-slate-700">
                <div>
                  <span className="font-medium text-white">{skill.name}</span>
                  {skill.level && <span className="text-sm text-slate-400 ml-2">({skill.level})</span>}
                </div>
                <div className="flex gap-2">
                  <button onClick={() => { setEditingItem(skill); setFormData(skill); }} className="text-blue-400 hover:text-blue-300">Edit</button>
                  <button onClick={() => handleDelete(skill.id)} className="text-red-400 hover:text-red-300">Delete</button>
                </div>
              </div>
            ))}
          </div>
